/*
    promise 객체 : 
    - 해당 객체가 생성된 시점에서 알려지지 않았을 수 있는 값을 위한 대리자
...
// // 비동기 연산이 종료된 이후에 결과 값과 실패에 대한 처리기 -> 맨 마지막에 처리함
// promise
//     .then((result) => {console.log(result)}) // 성공시
//     .catch((result) => {console.log(`catch : ${result}`)}); // 실패시
